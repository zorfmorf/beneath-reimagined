...
local function placeable(x, y, candidate)
    if candidate:instanceof(Cave()) then
        ...
        if tile(x-1,y) and tile(x-1,y).build and tile(x-1,y).build then return true end
        if tile(x+1,y) and tile(x+1,y).build and tile(x+1,y).build then return true end
        if tile(x,y-1) and tile(x,y-1).build and tile(x,y-1).build then return true end
        if tile(x,y+1) and tile(x,y+1).build and tile(x,y+1).build then return true end
        return false
    end
